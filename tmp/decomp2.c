...
unsigned char* decomp( unsigned char* table, 
		...
		do {
				if ( *data > 127 ){
					 ...
					 //	buf = decomp( table, table+(unsigned char)((*data-128)*2), table+((unsigned char)((*data-128)*2)+1), 
										buf, bufend  );
				} else {
						*buf = *data;
						buf++;
...
unsigned char* decompstacked42( unsigned char* table, 
		...
		union { unsigned char *data;
						unsigned short *shdata;
		} ud;
		ud.data=data;
		union { unsigned char *buf;
...
dataloop: __attribute__((hot)) //dataloop label
							//d = *data;
					if ( *ud.shdata & 0x8080 ){ // > 127
							u.sh = *ud.shdata; // todo. continue. with 64 bit operands.!
							// -> unroll / shifting
