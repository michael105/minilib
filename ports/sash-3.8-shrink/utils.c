...
)
{
	...
	{
		eprints( "Copying file \"%s\" to itself\n", srcName);
	...
	*retFileTable = NULL;
	...
	while ((dp = readdir(dirp)) != NULL)
	{
		...
		if (fileCount >=ARGMAXFILE)
		{
			...
			{
				ewrites( "Cannot allocate file list\n");
				ewrites( "Cannot allocate file list\n");
				closedir(dirp);
		...
		if (str == NULL)
		{
			ewrites( "No memory for file name\n");
			closedir(dirp);
...
#define MAXARG 256
	...
	*retArgv = NULL;
	...
	len = strlen(cmd) + 1;
	if ( len > MAXCMDLENGTH ){
			ewrites("Max cmd length exceeded\n");
			return FALSE;
	}
...
#define dbg(...) {}
  ...
  dbg("ma ab\n");	
		...
		if (quote)
		{
			ewrites( "Unmatched quote character\n");
  ...
  dbg("ma5\n");	
		...
		if (unquotedWildCards)
		{
			...
			if (fileCount == 0)
			{
				ewrites( "Wildcard expansion error\n");
	...
	dbg("d55 HOME: %s\n",getenv("HOME"));
		...
		{
				ewrites("Exceeded MAXARG argument limit\n");
				exit(1);
#ifndef MAXARG
			...
			if (newArgTable == NULL)
			{
				ewrites( "No memory for arg list\n");
...
)
{
	...
	while (argc-- > 0)
	{
		...
		if (len >= bufLen)
		{
			ewrites( "Argument string too long\n");
...
trySystem(const char * cmd)
{
	...
	if (status == -1)
		fprints( stderr, "Error starting command: %s\n", cmd);
