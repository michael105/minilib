...
do_exit(int argc, const char ** argv)
{
	...
	if (getpid() == 1)
	{
		ewrites( "You are the INIT process!\n");
...
do_setenv(int argc, const char ** argv)
{
	...
	if (str == NULL)
	{
		ewrites( "Cannot allocate memory\n");
...
do_cd(int argc, const char ** argv)
{
	...
	else
	{
		...
		if (path == NULL)
		{
			ewrites( "No HOME environment variable\n");
...
do_pwd(int argc, const char ** argv)
{
	...
	{
		ewrites( "Cannot get current directory\n");
...
do_ln(int argc, const char ** argv)
{
	...
	if (argv[1][0] == '-')
	{
		...
		if (argc != 4)
		{
			ewrites( "Wrong number of arguments for symbolic link\n");
		...
		return 0;
#else
		ewrites( "Symbolic links are not allowed\n");
...
do_mount(int argc, const char ** argv)
{
	...
	while ((argc > 0) && (**argv == '-'))
	{
		...
		while (*++str) switch (*str)
		{
			case 't':
				if ((argc <= 0) || (**argv == '-'))
				{
					ewrites( "Missing file system type\n");
	...
	if (argc != 2)
	{
		ewrites( "Wrong number of arguments for mount\n");
...
#elif	HAVE_BSD_MOUNT
	{
		...
		{
			eprints( "Unknown filesystem type: %s", type);
			ewrites(
...
do_where(int argc, const char ** argv)
{
	...
	{
		ewrites( "Program name cannot include a path\n");
	...
	{
		ewrites( "Memory allocation failed\n");
...
do_losetup(int argc, const char ** argv)
{
	...
	if (!strcmp(argv[1], "-d"))
	{
		...
		{
			eprints( "Error unassociating device: %s\n", 
				strerror(errno));
	...
	{
		eprints( "Error setting up loopback device: %s\n", 
			strerror(errno));
