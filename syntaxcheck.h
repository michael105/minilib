...
static inline int volatile __attribute__((always_inline)) select(int fd, volatile fd_set* readfd, volatile fd_set *writefd, volatile fd_set *exceptfd, volatile struct timeval *wait){
  int ret;
  register long int r10 asm ("r10") = (unsigned long int)exceptfd ; register long int r8 asmIn file included from ./minilib.h:1600,
                 from /dev/fd/63:477,
                 from <command-line>:
...
./compat/ioctl.h:16:2: [01;33mwarning: #warning or use the supplied script mini-gcc [-Wcpp][00m
   16 | #warning or use the supplied script mini-gcc
      | [00;35m ^~~~~~~[00m
 ("r8") = (unsigned long int) wait ; asm volatile ("syscall" : "=a" (ret) : "a" ( (23 | 0 ) ) , "D" (&fd), "S" (readfd), "d" ((unsigned long int)writefd), "r" (r10), "r" (r8) : "memory","rcx", "r11" );
  return(ret);
}
